...
class BodyLine {
  ...
  _replaceVar(varName, replacement) {
...
class SDFMaterialGlParamsConfig extends NodeParamsConfig {
  constructor() {
    super(...arguments);
    // globals
    this.globals = ParamConfig.FOLDER();
    this.color = ParamConfig.COLOR([1, 1, 1]);
    this.useEnvMap = ParamConfig.BOOLEAN(0);
    this.useReflection = ParamConfig.BOOLEAN(0);
    this.useRefraction = ParamConfig.BOOLEAN(0);
    // lighting
    this.lighting = ParamConfig.FOLDER();
    this.useLights = ParamConfig.BOOLEAN(1);
    this.diffuse = ParamConfig.COLOR([1, 1, 1], {
      visibleIf: { useLights: 1 }
    });
    this.emissive = ParamConfig.COLOR([0, 0, 0]);
    // envMap
    this.envMap = ParamConfig.FOLDER();
    this.envMapTint = ParamConfig.COLOR([1, 1, 1], {
      visibleIf: { useEnvMap: 1 }
    });
    this.envMapIntensity = ParamConfig.FLOAT(1, {
      visibleIf: { useEnvMap: 1 }
    });
    this.envMapRoughness = ParamConfig.FLOAT(0, {
      visibleIf: { useEnvMap: 1 }
    });
    this.envMapFresnel = ParamConfig.FLOAT(0, {
      visibleIf: { useEnvMap: 1 }
    });
    this.envMapFresnelPower = ParamConfig.FLOAT(5, {
      range: [0, 10],
      rangeLocked: [true, false],
      visibleIf: { useEnvMap: 1 }
    });
    // reflection
    this.reflection = ParamConfig.FOLDER();
    this.reflectionTint = ParamConfig.COLOR([1, 1, 1], {
      visibleIf: { useReflection: 1 }
    });
    this.reflectionDepth = ParamConfig.INTEGER(3, {
      visibleIf: { useReflection: 1 },
      range: [0, 10],
      rangeLocked: [true, false]
    });
    this.reflectivity = ParamConfig.FLOAT(0.5, {
      visibleIf: { useReflection: 1 },
      range: [0, 1],
      rangeLocked: [true, false]
    });
    this.reflectionBiasMult = ParamConfig.FLOAT(2, {
      visibleIf: { useReflection: 1 },
      range: [0, 10],
      rangeLocked: [true, false]
    });
    // refraction
    this.refraction = ParamConfig.FOLDER();
    this.refractionTint = ParamConfig.COLOR([1, 1, 1], {
      visibleIf: { useRefraction: 1 }
    });
    this.ior = ParamConfig.FLOAT(1.45, {
      visibleIf: { useRefraction: 1 },
      range: [0, 2],
      rangeLocked: [true, false]
    });
    this.splitRGB = ParamConfig.BOOLEAN(0, {
      visibleIf: { useRefraction: 1 }
    });
    this.iorOffset = ParamConfig.VECTOR3([-0.01, 0, 0.01], {
      visibleIf: { useRefraction: 1, splitRGB: 1 }
    });
    this.transmission = ParamConfig.FLOAT(0.5, {
      visibleIf: { useRefraction: 1 },
      range: [0, 1],
      rangeLocked: [true, false]
    });
    this.absorption = ParamConfig.FLOAT(0.5, {
      visibleIf: { useRefraction: 1 },
      range: [0, 5],
      rangeLocked: [false, false]
    });
    this.refractionDepth = ParamConfig.INTEGER(3, {
      visibleIf: { useRefraction: 1 },
      range: [0, 10],
      rangeLocked: [true, false],
      separatorBefore: true
    });
    this.refractionMaxDist = ParamConfig.FLOAT(100, {
      visibleIf: { useRefraction: 1 },
      range: [0, 100],
      rangeLocked: [true, false],
      step: 1e-4
    });
    this.refractionBiasMult = ParamConfig.FLOAT(2, {
      visibleIf: { useRefraction: 1 },
      range: [0, 10],
      rangeLocked: [true, false]
    });
    this.sampleEnvMapOnLastRefractionRay = ParamConfig.BOOLEAN(1, {
      visibleIf: { useRefraction: 1 }
    });
    this.refractionStartOutsideMedium = ParamConfig.BOOLEAN(1, {
      visibleIf: { useRefraction: 1 }
    });
  }
}
const ParamsConfig = new SDFMaterialGlParamsConfig();
export class SDFMaterialGlNode extends TypedGlNode {
  constructor() {
    super(...arguments);
  ...
  setLines(shadersCollectionController) {
    ...
    const bodyLines = [`if(mat == ${matIdName}){`];
