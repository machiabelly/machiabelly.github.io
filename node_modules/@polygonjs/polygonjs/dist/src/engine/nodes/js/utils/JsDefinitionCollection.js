      ...
      if (!this._errored) {
        ...
        if (existing) {
          if (existing.dataType() != definition.dataType()) {
            this._errored = true;
            this._error_message = `attempt to create '${definition.name()}' with types '${definition.dataType()}' by node '${definition.node().path()}', when there is already an existing with type ${existing.dataType()} from node '${existing.node().path()}'`;
