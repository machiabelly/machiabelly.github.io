...
export class CADPipeSopNode extends CADSopNode {
  constructor() {
    super(...arguments);
    ...
    if (profilesObjects && pathObjects) {
      CadGC.withGC((r) => {
        ...
        for (let i = 0; i < minObjectsCount; i++) {
          ...
          const wirePath = CoreCadType.isWire(pathObject) ? pathObject.cadGeometry() : cadWireFromEdge(oc, pathObject.cadGeometry());
            ...
            if (isBooleanTrue(this.pv.cap)) {
              const addCap = (capShape, invert) => {
                ...
                if (cadGeometryTypeFromShape(oc, capShape) == CadGeometryType.WIRE) {
                  const wire = cadDowncast(oc, capShape);
