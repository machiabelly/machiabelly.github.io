...
function loadExampleData() {
  ...
  if (examplePath) {
    ...
    const exampleType = elements[0];
    switch (exampleType) {
      case "nodes": {
        const context = elements[1];
        const nodeType = elements[2];
        const mapForContext = examples.nodes[context];
        if (!mapForContext) {
          return redirectToExample();
        }
        const exampleContent = mapForContext[nodeType];
      ...
      case "expressions": {
        ...
        const exampleContent = examples.expressions[expressionName];
...
function addSourceLink(examplePath) {
...
function addExampleHTML(htmlContext) {
...
function makeExampleLinkActive(exampleName) {
...
function createExampleLinks() {
  ...
  for (let context of nodeContexts) {
    const typeMap = examples.nodes[context];
    const exampleNames = Object.keys(typeMap);
    for (let exampleName of exampleNames) {
      const linkElement = linkTemplate.cloneNode(true);
