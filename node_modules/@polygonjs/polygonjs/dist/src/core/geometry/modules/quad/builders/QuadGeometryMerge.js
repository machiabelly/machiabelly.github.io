...
function _attributesAreTheSame(quadObjects, entityClass) {
...
function _mergePointAttributes(quadObjects, target) {
...
function _mergePointAttribute(quadObjects, attributeName, addIndex, target) {
...
function _mergeAttributes(quadObjects, entityClass, target) {
...
function _mergeAttribute(quadObjects, attributeName, entityClass, target) {
  const firstAttribute = entityClass.attribute(quadObjects[0], attributeName);
  const { itemSize, isString } = firstAttribute;
  const values = [];
  for (const object of quadObjects) {
    const attribute2 = entityClass.attribute(object, attributeName);
...
export function quadGeomeryMerge(quadObjects) {
  if (!(_attributesAreTheSame(quadObjects, QuadPoint) && _attributesAreTheSame(quadObjects, QuadVertex) && _attributesAreTheSame(quadObjects, QuadPrimitive))) {
    return;
  }
