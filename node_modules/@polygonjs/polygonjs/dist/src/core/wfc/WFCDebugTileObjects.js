...
export function createDefaultUnresolvedTileObject() {
  const geometry = new BoxGeometry(SCALE, SCALE, SCALE);
  const material = new MeshBasicMaterial({ color: 16711935 });
  const mesh = new Mesh(geometry, material);
  mesh.name = UNRESOLVED_TILE_ID;
  return mesh;
}
export function addEmptyTileObjectAttributes(object) {
...
export function addErrorTileObjectAttributes(object) {
  CoreWFCTileAttribute.setIsErrorTile(object, true);
  object.name = ERROR_TILE_ID;
  return object;
}
export function addUnresolvedTileObjectAttributes(object) {
