...
var pathArg = (path2, opt = {}) => {
  ...
  path2 = (0, import_path2.resolve)(path2);
  const { root } = (0, import_path2.parse)(path2);
  ...
  if (platform_default === "win32") {
    const badWinChars = /[*|"<>?:]/;
    const { root: root2 } = (0, import_path2.parse)(path2);
...
var rimrafPosixDir = async (path2, opt, ent) => {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafPosixDir((0, import_path3.resolve)(path2, ent2.name), opt, ent2)))).reduce((a, b) => a && b, true);
  if (!removedAll) {
    return false;
  }
...
var uniqueFilename = (path2) => `.${(0, import_path5.basename)(path2)}.${Math.random()}`;
...
var rimrafMoveRemoveDir = async (path2, opt, ent) => {
  ...
  if (path2 === opt.tmp && (0, import_path5.parse)(path2).root !== path2) {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafMoveRemoveDir((0, import_path5.resolve)(path2, ent2.name), opt, ent2)))).reduce((a, b) => a && b, true);
  if (!removedAll) {
    return false;
  }
...
var tmpUnlink = async (path2, tmp, rm3) => {
  const tmpFile = (0, import_path5.resolve)(tmp, uniqueFilename(path2));
...
var rimrafMoveRemoveDirSync = (path2, opt, ent) => {
  ...
  let removedAll = true;
  for (const ent2 of entries) {
    const p = (0, import_path5.resolve)(path2, ent2.name);
...
var rimrafWindowsDir = async (path2, opt, ent, state = START) => {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafWindowsDir((0, import_path6.resolve)(path2, ent2.name), opt, ent2, s)))).reduce((a, b) => a && b, true);
  if (state === START) {
...
// src/cleanBuildCommon.ts
var pathsToRemove = ["dist/polygonjs/backup"];
function removePath(path2) {
  return new Promise(async (resolve6, reject) => {
    const result = await rimraf(path2);
    if (!result) {
      console.error(`Error removing the path ${path2}:`, result);
      return reject(result);
    }
    console.log(`Successfully removed ${path2}`);
    resolve6();
  });
}
async function removeAllPaths(paths) {
  return await Promise.all(paths.map(removePath));
}
async function cleanBuild() {
  await removeAllPaths(pathsToRemove).catch(() => {
    process.exit(1);
  });
}
